...
if __name__ == '__main__':
    ...
    restoredir = hp.restoredir + "-" + str(num)
    newsaver = tf.train.Saver(max_to_keep=None)
    sv = tf.train.Supervisor(logdir=restoredir, save_model_secs=0, global_step=g.global_step, saver=newsaver)
    # with sv.managed_session() as sess:

    with sv.managed_session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        sv.saver.restore(sess, tf.train.latest_checkpoint(restoredir))
        # sv.saver.restore(sess, tf.train.latest_checkpoint("de/logdir-1"))
        while 1:
